...
function Orders() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 pt-16 md:pt-16">
      {/* Sidebar */}
      <div className={`fixed left-0 top-16 z-40 h-full w-64 bg-white border-r border-gray-200 shadow-sm transition-transform duration-300 ${showSidebar ? "translate-x-0" : "-translate-x-full lg:translate-x-0"
        }`}>
        {/* Sidebar Header */}
          ...
          <nav className="space-y-2">
            {loading ? (
              ...
              <>
                <Button
                  variant="ghost"
                  className={`w-full justify-start gap-2 h-auto p-3 ${!selectedCategoryId
                ...
                {uniqueCategories.map((category) => {
                  ...
                  return (
                    <Button
                      key={category._id}
                      variant="ghost"
                      className={`w-full justify-start gap-2 h-auto p-3 ${selectedCategoryId === category._id
                        ? "bg-green-700 text-white shadow-sm"
                        : "hover:bg-gray-100 text-gray-700"
                        }`}
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-2 md:gap-4">
                {filteredDishes.map((dish) => (
                  <Card
                    key={dish._id}
                    className={`group relative cursor-pointer rounded-2xl overflow-hidden border border-gray-100 bg-gradient-to-br from-white to-gray-50 shadow-md hover:shadow-xl hover:scale-[1.02] transition-all duration-300 ${(getAvailableQty(dish) !== null && getAvailableQty(dish) <= 0) ? "opacity-60 cursor-not-allowed hover:scale-100" : ""
                      }`}
                    onClick={() => {
                  ...
                  >
                      ...
                      <div className="flex items-start justify-between mb-3">
                        ...
                        <div className="flex gap-2 items-center">
                          ...
                          {dish.isVeg !== undefined && (
                            <span
                              className={`px-2 py-0.5 text-[10px] md:text-xs rounded-full font-medium shadow-sm ${dish.isVeg ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"
                          ...
                          {(() => {
                            ...
                            return (
                              <span className={`px-2 py-0.5 text-[10px] md:text-xs rounded-full font-medium shadow-sm ${inStock === null ? "bg-gray-100 text-gray-600" : inStock ? "bg-blue-100 text-blue-700" : "bg-red-100 text-red-700"
                                }`} onClick={(e) => e.stopPropagation()}>
                                Qty: {qty === null ? "-" : qty}
                              </span>
                            );
                    ...
                    <button
                      ...
                      className={`absolute bottom-3 right-3 rounded-full p-2 shadow-md opacity-0 group-hover:opacity-100 transition-all duration-300 ${(getAvailableQty(dish) !== null && getAvailableQty(dish) <= 0)
